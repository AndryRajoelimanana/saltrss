...
def rssextractone(images, arcl, config, logfile, do_error=False):
    ...
    if os.path.isfile('b'+images): os.system('mv b'+images+' '+images)
...
def rssfluxdefine(images,calflux, do_error=False):
...
def rssstdred(stdlist,stdarclist, sens, config, logfile):
    ...
    instdlist = read_fromfile(stdlist)
    instdarclist  = read_fromfile(stdarclist)
...
def reduce(objlist,objarclist,stdlist='', stdarclist='', logfile='saltlongslit.log',
           parfile='/home/rajoelimananaa/software/saltpy/saltparam.txt', do_error=True):

    inobjlist = read_fromfile(objlist)
    inobjarclist = read_fromfile(objarclist)
    ...
    for inobj in inobjlist:
        ...
        if (os.path.splitext(stdlist)[0] != stdlist and os.path.isfile(stdlist)):
            ...
            rssfluxdefine(inobj, stdlist, do_error=do_error)
